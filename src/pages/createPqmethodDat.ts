const createPQMethodDAT = (
  ...
  //   for (let j = 0; j < sorts[0].length; ++j) {
  //     const name = `statement ${j + 1}`;
  ...
  const sortsTextArray: string[] = sorts.map((item) =>
    item.reduce((sortText, value) => sortText + value.toString().padStart(2, " "), ""),
  );

  const participantNamesText: string[] = sorts.map(
    (_, index) => `${participantNames[index].padEnd(10, " ")}${sortsTextArray[index]}\n`,
  );
